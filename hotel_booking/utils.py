...
try:
	...
	from django.utils.translation import \
		gettext_lazy as _  # Django 4.0.0 and more
	...
	for model, model_admin in admin_site._registry.items():
		...
		if has_module_perms:
			...
			if True in perms.values():
				...
				if not app_icon:
					app_icon = default_apps_icon[
						app_label ] if app_label in default_apps_icon else None
...
def get_possible_language_codes():
	...
	if len( split ) == 2:
		language_code = '%s-%s' % (split[ 0 ].lower(), split[ 1 ].upper()) if \
		split[ 0 ] != split[ 1 ] else split[ 0 ]
...
def get_original_menu_items( context ):
	if context.get( 'user' ) and user_is_authenticated( context[ 'user' ] ):
		# pinned_apps = PinnedApplication.objects.filter(user=context[
        # 'user'].pk).values_list('app_label', flat=True)
		pinned_apps = [ ]
	else:
		pinned_apps = [ ]
...
def get_menu_item_url( url, original_app_list ):
	if isinstance( url, dict ):
		...
		if url_type == 'app':
			...
			return reverse( url[ 'name' ], args=url.get( 'args' ),
...
def get_menu_items( context ):
    ...
    # 'user'].pk).values_list('app_label', flat=True)
	pinned_apps = [ ]
	original_app_list = OrderedDict( map( lambda app: (app[ 'app_label' ],
                                                       app),
	...
	if custom_app_list not in (None, False):
		...
		def get_menu_item_app_model( app_label, data ):
			...
			if 'url' in data:
				item[ 'url' ] = get_menu_item_url( data[ 'url' ],
				                                   original_app_list
				                                   )
			...
			if 'permissions' in data:
				item[ 'has_perms' ] = (item.get( 'has_perms', True ) and
                                       context[
					'user' ].has_perms( data[ 'permissions' ] ))
		...
		def get_menu_item_app( data ):
			...
			if 'items' in data:
				item[ 'items' ] = list(
					map( lambda x: get_menu_item_app_model( app_label, x ),
					     data[ 'items' ]
					     )
					)
	...
	for app in app_list:
		if not current_found:
			for model in app[ 'items' ]:
				if not current_found and model.get( 'url' ) and context[
					'request' ].path.startswith( model[ 'url' ] ):
					model[ 'current' ] = True
					current_found = True
				else:
					model[ 'current' ] = False

			if not current_found and app.get( 'url' ) and context[
				'request' ].path.startswith( app[ 'url' ] ):
				app[ 'current' ] = True
				current_found = True
			else:
				app[ 'current' ] = False
